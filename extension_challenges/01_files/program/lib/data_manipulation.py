...
def christmas_day_air_quality(filename, include_header_row):
    file_data = get_file_contents(filename)
    data_with_contents = [line for line in file_data if line != None]
    print(data_with_contents)
    xmas_data = [line for line in file_data if line != None and line[0][:5] == 25/12]
    print(xmas_data)
